...
from .models import PositionMatch, Match


@receiver(user_logged_in)
def get_user_matches_receiver(sender, request, user, *args, **kwargs):
    for u in get_user_model().objects.exclude(username=user.username).order_by("-id")[:200]:
        profile = Profile.objects.get_or_create(user=u)
        matched, created = Match.objects.get_or_create_match(
            user_a=u, user_b=user)
